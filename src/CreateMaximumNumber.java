...
public class CreateMaximumNumber {
  	public static int[] maxNumber(int[] nums1, int[] nums2, int k) {
  		...
  		for (int i = Math.max(k - nums2.length, 0); i <= get_from_nums1; i++) {
  			...
  			if (!compare(ans, 0, res, 0)) ans = res;
  		}
  		return ans;
    }
  	public static boolean compare(int[] nums1, int start1, int[] nums2, int start2) {
  	...
  	public static int[] solve(int nums[], int k) {
  		...
  		for (int i = 0; i < nums.length; i++) {
  			 while (len > 0 && len + nums.length - i > k && res[len - 1] < nums[i]) len--;
  	...
  	public static void main(String args[]) {
  		...
  		int k = 5;
  		int res[] = maxNumber(nums2, nums1, k);
